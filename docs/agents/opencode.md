...
OpenCode 通常支持在会话中切换 provider,具体命令以版本而定。常见做法是 `/provider nextoken` 之类的内置命令。
...
`config.json` 里 `default_provider` 的值必须和 `providers` 下的某个键完全一致(本例都是 `nextoken`)。区分大小写。
...
```json
{
  ...
  "model": "nextoken/gpt-5.4",
  "small_model": "nextoken/gpt-5.4-mini",
  "provider": {
    "nextoken": {
...
- `claude-opus-4-6` 和 `claude-sonnet-4-6` 的 `low / medium / high` 已用真实 `opencode run` 验证可正常调用；这只能说明当前 nextoken 接法接受这些 variants，不等同于 Anthropic 原生 `thinking` budget 档位声明。
