...
1. 先按 [创建 API Key 教程](https://docs.nextoken.online/nexttoken/create-key) 生成一个新的 API Key。
...
1. 按照 [创建 API Key 教程](https://docs.nextoken.online/nexttoken/create-key) ，获取一个新的 API Key。
2. 打开 `CC-Switch`，点击右上角的“添加”按钮，选择“添加统一供应商”。
3. 在弹出的配置窗口中，准确填写以下字段：

   ```text
   供应商名: nextoken
   API 地址: https://nextoken.online
...
# 切到便宜的渠道做大批量任务
ccswitch use nextoken
claude

# 用一个隔离的 Key 跑实验,避免污染主账户额度
ccswitch use nextoken-test
claude
```
...
- [Claude Code 使用指南](https://docs.nextoken.online/agents/claude-code)
- [Codex 使用指南](https://docs.nextoken.online/agents/codex)
- [OpenCode 使用指南](https://docs.nextoken.online/agents/opencode)
